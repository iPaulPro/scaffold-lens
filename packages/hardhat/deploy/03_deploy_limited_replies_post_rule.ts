...
const deployTestToken: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Load contract
  const artifact = await deployer.loadArtifact("LimitedRepliesPostRule");

  const contract = await deployer.deploy(artifact);
  const address = await contract.getAddress();
  console.log("LimitedRepliesPostRule deployed to:", address);
...
deployTestToken.tags = ["LimitedRepliesPostRule"];
